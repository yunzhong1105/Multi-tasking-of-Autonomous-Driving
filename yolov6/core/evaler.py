...
def compute_iou(prediction, ground_truth, num_classes):
    ...
    for class_id in range(num_classes):
        intersection = np.logical_and(prediction == class_id, ground_truth == class_id)
        union = np.logical_or(prediction == class_id, ground_truth == class_id)
...
def compute_miou(predictions, ground_truths, num_classes):
    """
    計算預測的分割圖像和對應的真實標籤的平均IoU值
    """
    miou_scores = np.zeros(num_classes)
    num_images = len(predictions)
    for i in range(num_images):
        iou_scores = compute_iou(predictions[i], ground_truths[i], num_classes)
        miou_scores += iou_scores
    miou_scores /= num_images
    ...
    def init_data(self, dataloader, task):
        ...
        if task != 'train':
            ...
            dataloader = create_dataloader(self.data[task if task in ('train', 'val', 'test') else 'val'],
                                           ...
                                           data_dict=self.data, task=task, args=self.args)[0]
        return dataloader
...
#             pdb.set_trace()
            ...
            if self.args.detonly != 'True':
                ...
                m1=0
                for s, g in zip(segmap, segs):
                    s = torch.argmax(s,0)
    #                 import pdb
    #                 pdb.set_trace()
                    m1+=compute_miou(s.cpu().numpy(), g.cpu().numpy(), 6)
                miou1.append( m1/segs.shape[0])
    #             print(np.mean(np.array(miou1)))
            
                if not os.path.isdir('results'):
                    os.mkdir('results')
                for s,pa, img in zip(segmap,paths, imgs):
                    s = s.cpu().numpy()
                    s = np.argmax(s,0)
                    cv2.imwrite('results/'+os.path.basename(pa)+'.png', s.astype(np.uint8))
                    color_pred_segmap = segmap_to_color(s)
                    color_image = colorize_image(img, s)
                    cv2.imwrite('results/'+os.path.basename(pa)+'_color.png', color_image)
        ...
        if self.do_pr_metric and self.args.segonly != 'True':
            # Compute statistics
