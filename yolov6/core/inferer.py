...
#     import pdb
#     pdb.set_trace()
    result = cv2.addWeighted(image, 0.5, color_segmap, 0.5, 0.0)
    # 转换为RGB图像
...
class Inferer:
    def __init__(self, source, webcam, webcam_addr, weights, device, yaml, img_size, half):
        ...
        # import inspect
        # print(inspect.getmembers(self.model , lambda a:not(inspect.isroutine(a))))
        # print(self.model)
        # assert 1 == 2
...
#                 LOGGER.info("Switch layer to deploy modality:"+str(layer))

        LOGGER.info("Switch model to deploy modality.")

    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, save_seg, hide_labels, hide_conf, view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            ...
            # print("after expand dim img shape :" , img.shape)
            t1 = time.time()
            ...
            t2 = time.time()
            # print("det: " , det)
            segmap = F.interpolate(segmap, size=(img_src.shape[:2]), mode='bilinear', align_corners=True)
...
#             pdb.set_trace()
            ...
            if not os.path.isdir('results_sub'):
                os.mkdir('results_sub')
            
            if save_seg:
                segmap = segmap[0].cpu().numpy()
                segmap = np.argmax(segmap,0)
                pa = img_path
                # print('results_sub/'+os.path.basename(pa).replace('jpg', 'png'))
                # print("cvpr_infer/output/seg/" + os.path.basename(pa).replace('jpg', 'png'))
                # assert 1 == 2
                cv2.imwrite("cvpr_infer/output/seg/" + os.path.basename(pa).replace('jpg', 'png'), segmap.astype(np.uint8))
                # cv2.imwrite('results_sub/'+os.path.basename(pa).replace('jpg', 'png'), segmap.astype(np.uint8))
                color_pred_segmap = segmap_to_color(segmap)
    #             color_image = colorize_image(img_src, segmap)
    #             cv2.imwrite('results_sub/'+os.path.basename(pa)+'_color.png', color_image)
            ...
            if self.webcam:
                ...
                txt_path = osp.join(save_dir, rel_path, "submission")
                # print("txt_path : \n" , txt_path)
...
#                 cv2.waitKey(1)  # 1 millisecond
    ...
    def process_image(img_src, img_size, stride, half):
        ...
        image = letterbox(img_src, None, img_size, stride=stride)[0]
        # Convert
