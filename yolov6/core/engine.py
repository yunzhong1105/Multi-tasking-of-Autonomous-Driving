...
def load_state_dict_with_skip(model, state_dict, skip_keys=[]):
    """
    Load a PyTorch state_dict while skipping specified keys.
    Args:
        model: The PyTorch model to load the state_dict into.
        state_dict: The state_dict to load into the model.
        skip_keys: A list of keys to skip when loading the state_dict.
    """
    ...
    for name, param in state_dict.items():
        ...
        if name not in own_state:
            print(f"Skipped loading parameter {name}: not found in model.")
            continue
        if isinstance(param, torch.nn.Parameter):
            # backwards compatibility for serialized parameters
            param = param.data
        own_state[name].copy_(param)
    missing_keys = set(own_state.keys()) - set(state_dict.keys()) - set(skip_keys)
    if len(missing_keys) > 0:
        print(f"Skipped loading {len(missing_keys)} parameters: {missing_keys}")

def setFreezeWeights(model, freeze_backbone, segonly, detonly):
    params_to_update = None
    
    if freeze_backbone==1 and (segonly=='True' or detonly=='True'):
            layers_to_freeze = 'detect'
            params_to_update = []
            for layer_name, layer in model.named_parameters():
                if layers_to_freeze not in layer_name:
#                     print(layer_name)
                    layer.requires_grad = False
                else:
                    if segonly=='True':
                    
                        if 'spp' in layer_name or 'seg_head' in layer_name:
                            params_to_update.append(layer)
                            # print(layer_name)
                        else:
                            layer.requires_grad = False
                            
                    if detonly=='True':
                        if 'spp' not in layer_name and 'seg_head' not in layer_name:
                            params_to_update.append(layer)
                            # print(layer_name)
                        else:
                            layer.requires_grad = False
                            
    elif freeze_backbone==2 and (segonly=='True' or detonly=='True'):
            layers_to_freeze = ['detect', 'neck']
            params_to_update = []
            for layer_name, layer in model.named_parameters():
                if layers_to_freeze[0] not in layer_name and layers_to_freeze[1] not in layer_name:
#                     print(layer_name)
                    layer.requires_grad = False
                else:
                    if segonly=='True':
                    
                        if 'spp' in layer_name or 'seg_head' in layer_name or 'neck' in layer_name:
                            params_to_update.append(layer)
                            # print(layer_name)
                        else:
                            layer.requires_grad = False
                    ...
                    if detonly=='True':
                        if 'spp' not in layer_name and 'seg_head' not in layer_name or 'neck' in layer_name:
                            params_to_update.append(layer)
                            # print(layer_name)
                        else:
                            layer.requires_grad = False
...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.cfg = cfg
        self.device = device
        global detonly, segonly, clsonly, args1
        detonly = args.detonly
        segonly = args.segonly
        clsonly = args.clsonly
        args1=args
...
#             args.epochs = args.epochs+100
        ...
        # print(args.img_size)
        # print("#"*80)
        # input_shape = next(iter(model.named_parameters()))[1].size()[1:]
        # print("模型期望的輸入大小:", input_shape)
        # print("Over")
        ...
        params_to_update = setFreezeWeights(model, args.freeze_backbone, segonly, detonly)
...
#             print('params_to_update:',params_to_update)
            self.optimizer = self.get_optimizer(args, cfg, model, params_to_update)
        ...
        if hasattr(self, "ckpt"):
            #load_state_dict_with_skip(model, pretrained_dict, skip_keys=["fc.weight", "fc.bias"])
            # detect.cls_preds.0.weight/ detect.cls_preds.0.bias
            skipdict = []
            for i in range(4):
                skipdict.append(f'detect.cls_preds.{i}.weight')
                skipdict.append(f'detect.cls_preds.{i}.bias')
            resume_state_dict = self.ckpt['model'].float().state_dict()  # checkpoint state_dict as FP32
            if self.args.skip=='False':
                model.load_state_dict(resume_state_dict, strict=True)  # loadp- 1
                print('='*80)
                print('Normal loading')
#                 self.optimizer.load_state_dict(self.ckpt['optimizer'])
            else:
                load_state_dict_with_skip(model, resume_state_dict, skip_keys=skipdict)
                print('='*80)
                print('Skip loading')
...
#                     self.ema.ema.load_state_dict(self.ckpt['ema'].float().state_dict())
                    ...
                    if self.ckpt['ema'] is not None:
                        load_state_dict_with_skip(self.ema.ema, self.ckpt['ema'].float().state_dict(), skip_keys=skipdict)
...
#             self.strip_model()
    ...
    #             print(self.model.module.detect.cls_preds[-1].weight.view(-1)*10)
            for self.step, self.batch_data in self.pbar:
                # print(self.batch_data[2])
...
#         pdb.set_trace()
            ...
            with amp.autocast(enabled=self.device != 'cpu'):
                ...
                elif self.args.fuse_ab: # not here
            ...
            # print(self.optimizer.param_groups[2].keys())
    ...
    def get_data_loader(args, cfg, data_dict):
        ...
        # print(train_path, args.img_size, args.batch_size , args.world_size, grid_size, dict(cfg.data_aug) , args.local_rank,
        #                                  args.workers, True, args.check_images,
        #                                  args.check_labels , sep = "  |  ")
        # assert False
        ...
        train_loader = create_dataloader(train_path, args.img_size, args.batch_size // args.world_size, grid_size,
                                         ...
                                         check_labels=args.check_labels, data_dict=data_dict, task='train', args=args)[0]
        ...
        if args.rank in [-1, 0]:
            val_loader = create_dataloader(val_path, args.img_size, args.batch_size // args.world_size * 2, grid_size,
                                           ...
                                           check_labels=args.check_labels, data_dict=data_dict, task='val', args=args)[0]
