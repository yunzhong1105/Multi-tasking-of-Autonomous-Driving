...
def build_optimizer(cfg, model, params=None):
    """ Build optimizer from cfg file."""
    g_bnw, g_w, g_b = [], [], []
    if params is None:
        print('#'*80)
        print('Use all params')
        ...
        print('#'*80)
        print('Use specific params')
        if cfg.solver.optim == 'SGD':
            optimizer = torch.optim.SGD(params, lr=cfg.solver.lr0, momentum=cfg.solver.momentum, nesterov=True)
        elif cfg.solver.optim == 'Adam':
            optimizer = torch.optim.Adam(params, lr=cfg.solver.lr0, betas=(cfg.solver.momentum, 0.999))
