...
def letterbox(im, seg, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleup=True, stride=32, return_int=False, args=None):
    ...
    if shape[::-1] != new_unpad:  # resize
        ...
        if args is not None and args.detonly!='True':
    ...
    if not return_int:
        return im, seg, r, (dw, dh)
    else:
        return im, seg, r, (left, top)


def mixup(im, labels, im2, labels2, seg, seg2,args):
...
def random_affine(img, segs, labels=(), degrees=10, translate=.1, scale=.1, shear=10,
                  new_shape=(640, 640), args=None):
    ...
    if (M != np.eye(3)).any():  # image changed
        ...
        if args.detonly != 'True':
            segs = cv2.warpAffine(segs, M[:2], dsize=(width, height), borderValue=(0, 0, 0))
...
def mosaic_augmentation(img_size, imgs, segs, hs, ws, labels, hyp, args):
    ...
    for i in range(len(imgs)):
        ...
        if args.detonly!='True':
            ...
            # print(seg)
            # print("#"*80)
            img4s[y1a:y2a, x1a:x2a] = seg[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
        padw = x1a - x1b
        padh = y1a - y1b
    ...
    # Augment
    img4, img4s, labels4 = random_affine(img4, img4s, labels4,
                                  degrees=hyp['degrees'],
                                  translate=hyp['translate'],
                                  scale=hyp['scale'],
                                  shear=hyp['shear'],
                                  new_shape=(img_size, img_size), args=args)

    return img4, img4s, labels4
