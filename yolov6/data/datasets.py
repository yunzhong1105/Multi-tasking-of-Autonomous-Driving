...
#             print('1',seg.max())
            ...
            if random.random() < self.hyp["mixup"]:
                img_other, seg_other, labels_other = self.get_mosaic(
...
#             print('3',seg.max())
            ...
            if self.hyp and "letterbox_return_int" in self.hyp:
                img, seg, ratio, pad = letterbox(img, seg, shape, auto=False, scaleup=self.augment, return_int=self.hyp["letterbox_return_int"], args=self.args)
            else:
                img, seg, ratio, pad = letterbox(img, seg, shape, auto=False, scaleup=self.augment, args=self.args)
...
#             print('6',seg.max())
    ...
    def load_image(self, index, force_load_size=None):
        ...
        # path2=path.replace('images', 'labels/class_labels').replace('.jpg','_lane_line_label_id.png')
        path3=path.replace('images', 'label').replace('jpg','png')

        # if not os.path.isfile(path2):
        #     path2= path2.replace('_lane_line_label_id','')
        # if not os.path.isfile(path2):
        #     path2=path.replace('/images', '/labels').replace('.jpg','.png')
...
#         print(path2)
    ...
    def get_imgs_labels(self, img_dir):
        ...
        if base_dir != "":
            ...
            # assert False
            # label_dir = osp.join(
            # osp.dirname(osp.dirname(img_dir)).replace("images" , "labels") , osp.basename(img_dir)
            # )
            # print("#"*80)
            # print(osp.dirname(osp.dirname(img_dir)))
            # print(img_dir)
            # print(label_dir)
            # print("#"*80)
            # assert 1 == 2
            # label_dir = img_dir.replace('images','class_txt')
            # print(self.args.detonly , self.args.segonly , self.args.clsonly)
            if self.args.detonly == "True" : # if self.args.segonly!='True':
...
#             print(rel_path)
    ...
    def general_augment(self, img, seg, labels):
