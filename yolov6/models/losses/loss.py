...
class ComputeLoss:
    ...
    def __init__(self, 
        ...
        self.formal_assigner = TaskAlignedAssigner(topk=3, num_classes=self.num_classes, alpha=1.0, beta=6.0)
    ...
    def __call__(
        ...
        # print(seg_gt.shape[1:])
        # print(F.interpolate(segmap, size=(seg_gt.shape[1:])).shape)
        # print(F.interpolate(segmap, size=(seg_gt.shape[1:])))
...
#             pdb.set_trace()
            ...
            try:
                if epoch_num < self.warmup_epoch:
                    ...
                    target_labels, target_bboxes, target_scores, fg_mask = \
                        self.formal_assigner(pred_scores.detach(), pred_bboxes.detach() * stride_tensor, anchor_points, gt_labels,gt_bboxes,  mask_gt)
        ...
        if args.segonly == 'True' :
            ...
            # assert 1 == 2
            lossseg = self.celoss(segmap, seg_gt)
            # print("segmap :" , segmap.shape)
            # assert 1 == 2
        ...
        if args.detonly == 'True' and args.segonly == 'True':
            ...
            # assert 1 == 2
            # loss_cls = torch.zeros((lossseg.shape)).to(device_id)
            # loss_iou = torch.zeros((lossseg.shape)).to(device_id)
            # loss_dfl = torch.zeros((lossseg.shape)).to(device_id)
